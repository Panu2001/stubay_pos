<div class="pos-lend-history-page">
    ...
    <div class="pos-lend-history-summary">
        <div class="pos-lend-history-stat">
            ...
            <strong>{{ $currency }}{{ number_format($totalLends, 2) }}</strong>
        </div>
        <div class="pos-lend-history-stat">
            <span>Total Paid</span>
            <strong>{{ $currency }}{{ number_format($totalPaid, 2) }}</strong>
        </div>
        <div class="pos-lend-history-stat">
                    ...
                    <tbody>
                        @foreach ($lends as $lend)
                            <tr>
                                ...
                                <td>{{ $lend->order?->order_number ?? 'No linked order' }}</td>
                                <td>
                                    <div class="pos-lend-history-items">
                                        @forelse ($lend->order?->items ?? [] as $item)
                                ...
                                <td class="{{ $lend->remaining_amount > 0 ? 'pos-lend-history-remaining' : 'pos-lend-history-paid' }}">
                                    {{ $currency }}{{ number_format((float) $lend->remaining_amount, 2) }}
                                </td>
                                <td>
                                    <span class="pos-lend-history-badge {{ $lend->status === 'paid' ? 'is-paid' : '' }}">
                                        {{ str($lend->status)->replace('_', ' ')->title() }}
                                    </span>
                                </td>
                                <td>{{ $lend->notes ?: '-' }}</td>
                            </tr>

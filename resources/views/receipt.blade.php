...
<body class="h-full bg-neutral-800 flex items-start justify-center overflow-auto p-6">
    <div class="receipt-wrapper w-full max-w-[300px] bg-white text-neutral-900 shadow-2xl relative" style="font-size:11px;">
        ...
        <div class="text-center pt-0 px-3 pb-1">
            @if($logo = \App\Models\Setting::get('store_logo'))
                <div class="flex justify-center mb-1">
                    <img src="{{ asset('storage/' . $logo) }}" class="h-9 w-auto" alt="Logo">
                </div>
            @endif
            <div class="text-base font-bold tracking-wide leading-tight">
                {{ \App\Models\Setting::get('store_name', 'Anantha Multishop') }}
            </div>
        ...
        <div class="px-3 py-1 text-[9px] text-neutral-600 flex flex-col space-y-0.5">
            <div>
                Date: <span class="text-neutral-800 font-medium">{{ $order->created_at->format('Y-m-d') }}</span>
            </div>
            <div>
                Time: <span class="text-neutral-800 font-medium">{{ $order->created_at->format('h:i A') }}</span>
            </div>
            <div>
                Receipt #: <span class="text-neutral-800 font-medium">{{ $order->order_number }}</span>
            </div>
            <div>
                Cashier: <span class="text-neutral-800 font-medium">{{ $order->user->name ?? 'System' }}</span>
            </div>
            @if($order->customer)
            <div>
                Customer: <span class="text-neutral-800 font-medium">{{ $order->customer->name }}</span>
            </div>
            @endif
        </div>
        ...
        <div class="px-3 py-0.5 space-y-0.5">
            ...
            <div class="flex text-[9.5px] leading-tight">
                <div class="w-[45%] pr-1">
                    <div class="font-medium whitespace-normal break-words leading-tight">{{ $item->custom_name ?: ($item->product ? $item->product->name : 'Deleted Product') }}</div>
                </div>
                <div class="w-[15%] text-center">
                    {{ $item->quantity }}
                    @if($item->product && !in_array(strtolower($item->product->unit), ['pice', 'box', 'packet', 'bottle', 'pkt']))
                        <span class="text-[7.5px]">{{ $item->product->unit }}</span>
                    @endif
                </div>
                <div class="w-[15%] text-right text-neutral-500">{{ number_format($item->unit_price, 2) }}</div>
                <div class="w-[25%] text-right font-bold">{{ number_format($item->subtotal, 2) }}</div>
            </div>
            @endforeach
        </div>
        ...
        <div class="px-3 space-y-0.5 text-[10px]">
            <div class="flex justify-between text-neutral-600">
                <span>Subtotal</span> <span class="font-medium text-neutral-800">{{ number_format($order->subtotal, 2) }}</span>
            </div>
            @if($order->discount > 0)
            <div class="flex justify-between text-neutral-600">
                <span>Discount</span> <span class="font-medium text-neutral-800">-{{ number_format($order->discount, 2) }}</span>
            </div>
            @endif
            <div class="flex justify-between text-neutral-600">
                <span>Tax ({{ $order->tax_rate ?: \App\Models\Setting::get('tax_rate', 5) }}%)</span> <span class="font-medium text-neutral-800">{{ number_format($order->tax, 2) }}</span>
            </div>
            
            <div class="dotted-line my-0.5"></div>
            
            <div class="flex justify-between font-bold text-xs py-0.5">
                <span>TOTAL</span> <span>{{ $currency }} {{ number_format($order->total, 2) }}</span>
            </div>
        </div>
        ...
        <div class="px-3 space-y-0.5 text-[10px]">
            ...
            <div class="flex justify-between text-neutral-600">
                <span>Amount Tendered</span> <span class="font-medium text-neutral-800">{{ number_format($order->amount_tendered, 2) }}</span>
            </div>
            <div class="flex justify-between font-bold">
                <span>Change</span> <span>{{ number_format($order->change, 2) }}</span>
            </div>
            @endif

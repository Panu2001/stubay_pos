...
<body class="h-full bg-neutral-800 flex items-start justify-center overflow-auto p-6">
    <div class="receipt-wrapper w-full max-w-[320px] bg-white text-neutral-900 shadow-2xl relative" style="font-size:12px;">
        ...
        <div class="text-center pt-6 px-4 pb-3">
            @if($logo = \App\Models\Setting::get('store_logo'))
                <div class="flex justify-center mb-2">
                    <img src="{{ asset('storage/' . $logo) }}" class="h-12 w-auto" alt="Logo">
                </div>
            @endif
        ...
        <div class="px-4 py-2 text-[10px] text-neutral-600 flex flex-col space-y-0.5">
            <div>
                Date: <span class="text-neutral-800 font-medium">{{ $purchase->created_at->format('Y-m-d') }}</span>
            </div>
            <div>
                Time: <span class="text-neutral-800 font-medium">{{ $purchase->created_at->format('h:i A') }}</span>
            </div>
            <div>
                Purchase #: <span class="text-neutral-800 font-medium">{{ $purchase->purchase_number }}</span>
            </div>
            <div>
                Processed By: <span class="text-neutral-800 font-medium">{{ $purchase->user->name ?? 'System' }}</span>
            </div>
            <div class="dotted-line my-1"></div>
            <div>
                Supplier: <span class="text-neutral-800 font-bold">{{ $purchase->supplier->name }}</span>
            </div>
            @if($purchase->supplier->phone)
            <div>
                Phone: <span class="text-neutral-800 font-medium">{{ $purchase->supplier->phone }}</span>
            </div>
            @endif
        </div>
        ...
        <div class="px-4 py-1 space-y-1.5">
            ...
            <div class="flex text-[10px]">
                <div class="w-[45%] pr-1">
                    <div class="font-medium whitespace-normal break-words leading-tight">{{ $item->product ? $item->product->name : 'Deleted Product' }}</div>
                </div>
                <div class="w-[15%] text-center">
                    {{ (float)$item->quantity }}
                    @if($item->product && !in_array(strtolower($item->product->unit), ['pice', 'box', 'packet', 'bottle', 'pkt']))
                        <span class="text-[8px]">{{ $item->product->unit }}</span>
                    @endif
                </div>
                <div class="w-[15%] text-right text-neutral-500">{{ number_format($item->unit_cost, 2) }}</div>
                <div class="w-[25%] text-right font-medium">{{ number_format($item->subtotal, 2) }}</div>
            </div>
            @endforeach
        </div>
        ...
        <div class="px-4 space-y-1 text-[11px]">
            <div class="flex justify-between font-bold text-sm">
                <span>TOTAL BILL</span> <span>{{ $currency }} {{ number_format($purchase->total_amount, 2) }}</span>
            </div>
            
            <div class="dotted-line my-1"></div>
            
            <div class="flex justify-between text-neutral-700">
                <span>Amount Paid</span> <span class="font-bold text-green-600">{{ number_format($purchase->paid_amount, 2) }}</span>
            </div>
            
            <div class="flex justify-between text-neutral-700">
                <span>Owed (Debt)</span> <span class="font-bold text-red-600">{{ number_format($purchase->owed_amount, 2) }}</span>
            </div>
        </div>
        ...
        <div class="px-4 text-[10px] text-neutral-500">
            Payment Method: <span class="font-medium text-neutral-700 uppercase">{{ $purchase->payment_method }}</span>
            @if($purchase->notes)
                <div class="mt-1 italic">Notes: {{ $purchase->notes }}</div>
            @endif
        </div>
